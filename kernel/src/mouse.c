...
static dmouse_packet_t process_packet(mouse_packet_t *p) {
    ...
    if (p->flags & (1 << 0)) {
        dp.mouse_click_l = DAWN_FIXED_ONE;
    } else {
        dp.mouse_click_l = 0;
    }

    if (p->flags & (1 << 1)) {
        dp.mouse_click_r = DAWN_FIXED_ONE;
    } else {
        dp.mouse_click_r = 0;
    }

    if (p->flags & (1 << 2)) {
        dp.mouse_click_m = DAWN_FIXED_ONE;
    } else {
        dp.mouse_click_m = 0;
    }
    ...
    dp.mouse_x = scale_position(0, vbe_width, 0, DAWN_FIXED_ONE, mouse_x);
    dp.mouse_y = scale_position(0, vbe_height, 0, DAWN_FIXED_ONE, mouse_y);
...
void mouse_update(void) {
    ...
    /* wait for Dawn to clear mouse registers */
    if (
        dawn_readram(DAWN_MOUSE_REL_X) ||
        dawn_readram(DAWN_MOUSE_REL_Y) ||
        dawn_readram(DAWN_MOUSE_ABS_X) ||
        dawn_readram(DAWN_MOUSE_ABS_Y) ||
        dawn_readram(DAWN_MOUSE_TOUCH) ||
        dawn_readram(DAWN_MOUSE_SCROLL)
    ) return;
    ...
    dawn_writeram(DAWN_MOUSE_CLICK_L, dawn_mouse_click_l);
    dawn_writeram(DAWN_MOUSE_CLICK_R, dawn_mouse_click_r);
    dawn_writeram(DAWN_MOUSE_CLICK_M, dawn_mouse_click_m);
    dawn_writeram(DAWN_MOUSE_ABS_X, dawn_mouse_x);
    dawn_writeram(DAWN_MOUSE_ABS_Y, dawn_mouse_y);
...
void mouse_init(void) {
    ...
    dawn_mouse_x = scale_position(0, vbe_width, 0, DAWN_FIXED_ONE, mouse_x);
    dawn_mouse_y = scale_position(0, vbe_height, 0, DAWN_FIXED_ONE, mouse_y);
